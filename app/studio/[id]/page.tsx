  ...
  if (profileUser === null) {
    return (
      <main className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-8 text-center">
        <h1 className="text-3xl font-preahvihear text-white mb-4">Studio not found</h1>
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <Container className="pt-24 pb-20">
             ...
             <div className="relative z-10 p-8 flex flex-col md:flex-row justify-between items-end gap-6">
                <div className="flex items-end gap-6 translate-y-8 md:translate-y-12">
                    ...
                    <div className="mb-4 pb-2">
                        ...
                        <p className="text-white/80 font-dm-sans text-lg drop-shadow-md capitalize">{profileUser.role || 'Studio'}</p>
                    </div>
                </div>
            ...
            >
              ...
              <div className="relative z-10 h-full flex flex-col">
                <div className="space-y-6 flex-1">
                    <div>
                        <h4 className="text-sm text-white/50 mb-2 uppercase tracking-wider font-semibold text-[10px]">Description</h4>
                        <p className="text-sm text-white/80 font-dm-sans leading-relaxed line-clamp-6">
                            {profileUser.bio || "Assuming the role of a studio in the Edenn ecosystem implies a commitment to innovation."}
                        </p>
                    </div>
        ...
        {/* BOTTOM SECTION: PORTFOLIO / GAMES */}
        <div className="mt-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {/* SHOW SNAPSHOTS IF NO GAMES */}
                {userGames.length === 0 && (profileUser.snapshots && profileUser.snapshots.length > 0) && (
                    ...
                    [1, 2, 3].map((_, i) => (
                        <div key={i} className="group relative aspect-video rounded-[24px] overflow-hidden border border-white/10 bg-[#111]">
                            ...
                            <div className="absolute inset-x-0 bottom-0 p-4 bg-linear-to-t from-black to-transparent flex justify-between items-center">
                                    <div>
                                    <p className="text-white font-bold text-sm">Studio Project {i+1}</p>
                                    <p className="text-white/40 text-[10px] uppercase tracking-wider">{profileUser.name}</p>
                                    </div>
                                    <button className="bg-[#7628DB] text-white text-[10px] px-3 py-1 rounded-full font-bold uppercase hover:bg-[#6020A0]">View</button>
                            </div>
                        </div>
                    ))
                )}
